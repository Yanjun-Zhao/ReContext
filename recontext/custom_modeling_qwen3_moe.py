...
class RescaleQwen3MoeAttention(nn.Module):
    ...
    def __init__(self, config: Qwen3MoeConfig, layer_idx: int):
        ...
        # Unlike Qwen3, Qwen3MoE uses sliding_window directly from config (no layer_types)
...
class RescaleQwen3MoeDecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: Qwen3MoeConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size

        self.self_attn = RescaleQwen3MoeAttention(config, layer_idx)

        # MoE vs dense layer selection based on config
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        # For the MoE layers, we need to unpack (hidden_states, router_logits)
...
class Qwen3MoePreTrainedModel(PreTrainedModel):
    ...
    _no_split_modules = ["RescaleQwen3MoeDecoderLayer"]
    ...
    _can_record_outputs = {
        "router_logits": None,  # OutputRecorder would be needed for this but we don't need it for rescaling
        "hidden_states": RescaleQwen3MoeDecoderLayer,
        "attentions": RescaleQwen3MoeAttention,
    }


@auto_docstring
class RescaleQwen3MoeModel(Qwen3MoePreTrainedModel):
    def __init__(self, config: Qwen3MoeConfig):
        ...
        self.layers = nn.ModuleList(
            [RescaleQwen3MoeDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        # Unlike Qwen3 with layer_types, Qwen3MoE uses sliding_window from config directly
        ...
        decoding_up_to = self.config.num_hidden_layers if layer_early_stopping is None else layer_early_stopping + 1
        for decoder_layer in self.layers[: decoding_up_to]:
...
@auto_docstring
class RescaleQwen3MoeForCausalLM(Qwen3MoePreTrainedModel, CustomGenerationMixin):
    ...
    def __init__(self, config):
        super().__init__(config)
        self.model = RescaleQwen3MoeModel(config)
        self.vocab_size = config.vocab_size
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        # Optionally skip logits computation for memory efficiency (e.g., during rescaling prefill)
        if not compute_logits:
            logits = None
        elif compute_last_logits_only:
        ...
        return MoeCausalLMOutputWithPast(
            loss=loss,
            aux_loss=None,  # We don't compute aux_loss during inference/rescaling
            logits=logits,
...
__all__ = [
    "RescaleQwen3MoeForCausalLM",
    "Qwen3MoeForQuestionAnswering",
    "Qwen3MoePreTrainedModel",
    "RescaleQwen3MoeModel",
