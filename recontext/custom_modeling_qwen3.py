# Adapted from transformers v4.57.3
...
class RescaleQwen3Attention(nn.Module):
...
class RescaleQwen3DecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: Qwen3Config, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size

        self.self_attn = RescaleQwen3Attention(config=config, layer_idx=layer_idx)
...
class Qwen3PreTrainedModel(PreTrainedModel):
    ...
    _no_split_modules = ["RescaleQwen3DecoderLayer"]
    ...
    _can_record_outputs = {
        "hidden_states": RescaleQwen3DecoderLayer,
        "attentions": RescaleQwen3Attention,
    }
...
@auto_docstring
class RescaleQwen3Model(Qwen3PreTrainedModel):
    def __init__(self, config: Qwen3Config):
        ...
        self.layers = nn.ModuleList(
            [RescaleQwen3DecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
...
@auto_docstring
class RescaleQwen3ForCausalLM(Qwen3PreTrainedModel, CustomGenerationMixin):
    ...
    def __init__(self, config):
        super().__init__(config)
        self.model = RescaleQwen3Model(config)
        self.vocab_size = config.vocab_size
